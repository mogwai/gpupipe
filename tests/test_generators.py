...
def run_pipeline(stages: list[tuple], timeout: float = 60, debug: bool = False) -> list[dict]:
    """Run pipeline and collect results."""
    pipe = Pipe(debug=debug, raise_errors=True, stats_interval=0, health_check_interval=0)
...
class TestYieldingRootGenerator:
    ...
    def test_yield_root_debug_mode(self):
        """Yielding root generator works in debug mode."""
        n_items = 20
        stages = [
            (YieldingRootGenerator(n_items), {"outqn": 50}),
            (Collector(), {"workers": 1, "outqn": 0}),
        ]
        results = run_pipeline(stages, debug=True)
    ...
    def test_yield_root_process_mode(self):
        ...
        results = run_pipeline(stages, debug=False)
        assert len(results) == n_items
        assert {r["id"] for r in results} == set(range(n_items))
...
class TestYieldingMiddleWorker:
    ...
    def test_yield_expander_debug_mode(self):
        """Yielding expander works in debug mode."""
        n_items = 10
        factor = 3
        stages = [
            (YieldingRootGenerator(n_items), {"outqn": 50}),
...
class TestCombinedYieldPatterns:
    ...
    def test_yield_root_and_expander_debug(self):
        """Both root and middle use yield - debug mode."""
        n_items = 5
        factor = 4
        stages = [
            (YieldingRootGenerator(n_items), {"outqn": 50}),
